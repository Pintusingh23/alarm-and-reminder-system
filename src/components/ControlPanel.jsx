...
const ControlPanel = ({ searchQuery, setSearchQuery, sortBy, setSortBy, onClearAll }) => {
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-orange-100 animate-slide-up">
      ...
      <h2 className="text-lg font-bold mb-5 flex items-center gap-2.5">
        <div className="p-2 rounded-xl bg-gradient-to-br from-teal-100 to-emerald-100 border border-teal-200">
          <SlidersHorizontal className="text-teal-600" size={18} />
        </div>
        <span className="text-gray-800">Controls</span>
      </h2>
      ...
      <div className="space-y-4">
        ...
        {/* Search */}
        <div>
          <label className="block text-[10px] font-bold mb-2 text-gray-500 uppercase tracking-[0.15em]">
            Search
          </label>
          ...
          <div className="relative">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-black text-gray-800 placeholder-gray-400 text-sm focus:ring-2 focus:ring-teal-400 focus:outline-none transition-all"
              placeholder="Search reminders..."
            />
          </div>
        </div>
        ...
        <button
          onClick={onClearAll}
          className="w-full bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-2.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 border border-red-200 hover:border-red-300 text-sm shadow-sm hover:shadow-md"
        >
          <Trash2 size={14} />
          Clear All
        </button>
